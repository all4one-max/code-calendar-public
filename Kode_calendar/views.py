...
def make_api_calls():
    ...
    for contests in all_contest.objects.all():
        ...
        if md.get(st[1],-1)!=-1:
            ...
            start_time = datetime(int(st[2]), ccmd[st[1]], int(st[0]), int(st[3]), int(st[4]), 0)
        et = re.split("\W+",contests.end_time)        
        if md.get(et[1],-1)!=-1:
            end_time = datetime(int(et[2]), md[et[1]], int(et[0]), int(et[3]), int(et[4]), 0)
        else:
            end_time = datetime(int(et[2]), ccmd[et[1]], int(et[0]), int(et[3]), int(et[4]), 0)
        # print(contests.name,start_time,end_time)
        dic={"name":contests.name,"url":contests.url,"start_time":start_time,"end_time":end_time}
...
def get_local_time(time):  
    ...
    if (md.get(time[1],-1)==-1):
        utc_time = datetime(int(time[2]), ccmd[time[1]], int(time[0]), int(time[3]), int(time[4]), 0)
    else:
        utc_time = datetime(int(time[2]), md[time[1]], int(time[0]), int(time[3]), int(time[4]), 0)
...
flow = Flow.from_client_secrets_file(JSON_FILEPATH, scopes=scopes,redirect_uri = REDIRECT_URI)
contest_name = ""
...
def add_contest_to_calendar():
    for contests in all_contest.objects.all():
        if contest_name==contests.name:
            return contests.url,get_local_time(contests.start_time),get_local_time(contests.end_time)
